...
export default function IrrigationRecommendation() {
    ...
    const loadData = async () => {
      try {
        const [profileRes, stressRes] = await Promise.all([
          getFarmProfile(farmId).catch(() => ({ data: null })),
          checkStress({ farm_id: farmId }).catch(() => ({ data: null }))
        ]);
        if (profileRes.data) setFarmProfile(profileRes.data);
  ...
  // Derived Values
  const moisturePct = stressData ? Math.round(Math.max(10, stressData.ndwi_value * 80)) : 0;
  const crop = farmProfile?.current_crop || 'Unknown Crop';
  const size = farmProfile?.land_size_acres || 0;
  const stage = farmProfile?.crop_stage || 'Unknown Stage';
  const soil = farmProfile?.soil_type?.replace('_', ' ') || 'Unknown Soil';
  ...
  if (moisturePct === 0) {
    ...
    stressDesc = `No recent telemetry data available for this farm to determine soil moisture.`;
  } else if (moisturePct < 25) {
  ...
  const estCost = Math.round(size * 78.5);
  ...
  return (
    <AppShell 
      ...
      hideBottomNav={false}
      headerRightSlot={
        <button onClick={() => setAudioVisible(!audioVisible)} className="p-2 rounded-full active:bg-surface-container" type="button">
    ...
    >
      <div className="flex flex-col relative pb-28">
        ...
        <div className={`absolute top-2 inset-x-margin z-40 transition-all duration-300 ${audioVisible ? 'translate-y-0 opacity-100' : '-translate-y-8 opacity-0 pointer-events-none'}`}>
        ...
        <main className="flex flex-col w-full pt-6 px-margin bg-background flex-1 space-y-space-md">
          {/* Context Headline */}
          <div className="flex flex-col gap-1">
            ...
            <h2 className="font-headline-md text-headline-md text-on-surface capitalize">{crop} ({size} Ac)</h2>
            <span className="font-body-sm text-body-sm text-on-surface-variant capitalize">Stage: {stage}</span>
          </div>
          ...
          <section className="bg-surface-container rounded-xl p-space-md shadow-sm flex items-start gap-space-sm">
        ...
        {/* Action Dock */}
        <div className="px-margin mt-space-md flex flex-col gap-2">
          <button
            onClick={handleIrrigate}
            disabled={irrigateState !== 'idle' || moisturePct === 0}
